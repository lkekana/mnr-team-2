...
import {
  ArrowRight, Home, LayoutDashboard, Settings,
  User, BarChart2, PieChart,
} from "lucide-react";
...
const Dashboard = () => {
  ...
  const bottomLabel = "Current Score";
  ...
  return (
    <div className="min-h-screen flex bg-gray-100">
      ...
      {/* Main Content */}
      <div className="flex-1 flex flex-col items-center justify-center p-4 ml-6">
        {/* Center Circle */}
        <div className="relative w-64 h-64 rounded-full bg-white shadow-xl flex items-center justify-center">
          <div className="absolute top-4 text-sm text-green-600 font-semibold">
            +{incrementValue}
          </div>
          <div className="text-5xl font-bold text-gray-800">{mainValue}</div>
          <div className="absolute bottom-4 text-sm text-gray-500">{bottomLabel}</div>
        </div>

        {/* Grid Below */}
        <div className="mt-8 w-full max-w-4xl grid grid-cols-2 md:grid-cols-3 gap-4">
          {gridItems.map((item, index) => (
            <div
              key={index}
              onClick={item.onClick} // ✅ Here’s the important part
              className="bg-white px-4 py-3 shadow rounded-lg flex items-center justify-between cursor-pointer hover:bg-gray-50 transition"
            >
